...
const availableRooms = computed(() => {
  return getRoomsForBuilding(formsStore.fgmuState.sectionA.college_building);
});
...
<template>
  <div class="bg-white rounded-2xl sm:rounded-[3rem] border border-slate-200 shadow-2xl shadow-slate-200/40 overflow-hidden slide-up">
    <div class="bg-slate-900 p-5 sm:p-10 text-white flex flex-col sm:flex-row justify-between items-start sm:items-end gap-3 sm:gap-4 border-b-4 border-emerald-500">
      <div>
        <h3 class="text-xl sm:text-3xl font-black tracking-tight">Facilities Management <span class="text-emerald-500">(FGMU)</span></h3>
        <p class="text-slate-400 mt-1 text-xs sm:text-base font-medium">Structure, Utilities, and Mechanical requests.</p>
      </div>
    </div>

    <div class="p-4 sm:p-10 space-y-6 sm:space-y-12">
      <div class="bg-emerald-50/50 p-4 sm:p-8 rounded-2xl sm:rounded-[2rem] border border-emerald-100/60">
        <h4 class="text-xs font-black text-emerald-800 uppercase tracking-wider mb-3">Service(s) chosen will be handled by Facilities Management Unit</h4>
        <div class="flex flex-wrap gap-2">
          <div v-for="s in services" :key="s.service" class="px-3.5 py-2 bg-white border border-emerald-200/80 rounded-xl text-xs font-bold text-emerald-800 shadow-xs flex items-center gap-2">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-8">
        ...
        <div class="space-y-2 relative pb-5">
          <div class="flex items-center justify-between ml-1">
            <label class="text-xs font-bold uppercase tracking-wider" :class="formsStore.v$.fgmuState.sectionA.college_building.$error ? 'text-red-500' : 'text-slate-700'">
              College / Building
            </label>
        ...
        <div class="space-y-2 relative pb-5">
          <div class="flex items-center justify-between ml-1">
            <label class="text-xs font-bold uppercase tracking-wider" :class="formsStore.v$.fgmuState.sectionA.office_room.$error ? 'text-red-500' : 'text-slate-700'">
              Office / Room
            </label>
            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Pick or Type</span>
          </div>
        ...
        <div class="space-y-2 relative pb-5">
          ...
          <input v-model="formsStore.fgmuState.sectionA.source_of_fund" type="text" placeholder="e.g. Income, General Fund" class="w-full min-h-[48px] h-12 sm:h-14 px-4 sm:px-6 rounded-xl sm:rounded-2xl bg-slate-50 border-2 border-slate-100 focus:border-emerald-500 focus:bg-white text-base sm:text-sm font-bold outline-none transition-all shadow-xs" />
          <p class="text-[11px] text-slate-500 font-medium ml-1 flex items-center gap-1.5 leading-tight">
            <svg class="w-3.5 h-3.5 text-slate-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
            </svg>
      ...
      <div class="space-y-6">
        ...
        <div class="space-y-2 relative pb-4">
          <label class="text-xs font-bold uppercase tracking-wider ml-1" :class="formsStore.v$.fgmuState.sectionA.job_description.$error ? 'text-red-500' : 'text-slate-700'">Job Description / Particulars</label>
          <textarea 
            ...
            :class="formsStore.v$.fgmuState.sectionA.job_description.$error ? 'border-red-500 focus:border-red-500 text-red-900' : 'border-slate-100 focus:border-emerald-500'" 
          ></textarea>
        ...
        <div class="space-y-4 pt-1">
          ...
          <div 
            ...
            :class="isDragging ? 'border-emerald-500 bg-emerald-50/80 scale-[1.005]' : 'border-slate-200 bg-slate-50/50 hover:border-emerald-500 hover:bg-emerald-50/40'"
          >
            <input type="file" ref="fileInput" multiple @change="handleFile" class="hidden" accept="image/*,.pdf,.doc,.docx,.xls,.xlsx" />
            <div class="w-11 h-11 sm:w-12 sm:h-12 mx-auto mb-2.5 rounded-2xl bg-white border border-slate-200 group-hover:border-emerald-300 group-hover:bg-emerald-100/50 flex items-center justify-center text-slate-400 group-hover:text-emerald-700 transition-all shadow-xs">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
              </svg>
            </div>
            <p class="text-xs sm:text-sm font-black text-slate-700 group-hover:text-emerald-800 transition-colors">
          ...
          <div v-if="formsStore.fgmuState.attachments.length > 0" class="space-y-2 animate-fade-in pt-1">
            <div class="flex items-center justify-between text-xs font-bold text-slate-500 px-1">
              <span>Attached Files ({{ formsStore.fgmuState.attachments.length }})</span>
              <button type="button" @click="formsStore.fgmuState.attachments = []" class="text-red-500 hover:text-red-700 hover:underline cursor-pointer py-1 px-2">
                Remove All
              </button>
            </div>
            <div class="flex flex-wrap gap-2.5">
              <div 
                v-for="(f, i) in formsStore.fgmuState.attachments" 
                :key="f.name" 
