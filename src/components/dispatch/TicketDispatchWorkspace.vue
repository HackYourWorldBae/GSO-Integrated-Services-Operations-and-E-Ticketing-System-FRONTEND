<template>
  <div class="space-y-6 pb-12 px-4 sm:px-8 py-8 max-w-[1700px] mx-auto min-h-screen">
    <!-- Top Console Header -->
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div>
        <div class="flex items-center gap-2 mb-1">
          ...
          <span class="text-xs text-slate-400 font-bold tracking-tight">Interactive Dispatch Console</span>
        </div>
      ...
      <div class="flex flex-wrap items-center gap-2.5">
        <div class="flex items-center gap-2 px-3.5 py-2 bg-white rounded-xl border border-slate-200/80 shadow-xs">
          <span class="w-2.5 h-2.5 rounded-full bg-amber-500 animate-pulse"></span>
          <span class="text-xs font-bold text-slate-500">Pending Assignment:</span>
          <span class="text-xs font-black text-slate-900">{{ dispatchQueue.length }}</span>
        </div>

        <div class="flex items-center gap-2 px-3.5 py-2 bg-white rounded-xl border border-slate-200/80 shadow-xs">
          <span class="w-2.5 h-2.5 rounded-full bg-emerald-500"></span>
          <span class="text-xs font-bold text-slate-500">Available Staff:</span>
          <span class="text-xs font-black text-slate-900">{{ availableWorkersCount }} / {{ totalWorkersCount }}</span>
        </div>

        <router-link
          :to="`/admin/${unitCode.toLowerCase()}/personnel`"
          class="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl border border-slate-200 bg-white text-slate-700 text-xs font-black hover:bg-slate-50 hover:border-slate-300 transition-all shadow-xs cursor-pointer"
        >
        ...
        <router-link
          :to="`/admin/${unitCode.toLowerCase()}/dispatched`"
          class="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-xs font-black transition-all shadow-xs active:scale-95 cursor-pointer"
        >
    ...
    <div
      v-if="selectedTicket"
      class="p-4 sm:p-5 rounded-2xl bg-gradient-to-r from-slate-900 via-slate-850 to-slate-900 text-white shadow-lg border border-emerald-500/40 flex flex-col sm:flex-row sm:items-center justify-between gap-4 animate-fade-in"
    >
      <div class="flex items-center gap-3.5 min-w-0">
        <div class="h-10 px-3 min-w-[5rem] rounded-xl bg-emerald-500/20 border border-emerald-400/40 flex items-center justify-center shrink-0">
          <span class="text-xs font-black text-emerald-300">#{{ selectedTicket.id }}</span>
        </div>
        <div class="min-w-0">
          <div class="flex items-center gap-2 flex-wrap mb-0.5">
            <span class="px-2 py-0.5 rounded-md bg-emerald-500 text-white text-[9px] font-black uppercase tracking-wider">
              Active Assignment Target
            </span>
            <span class="text-xs text-slate-300 font-bold truncate">
          ...
          <p class="text-[11px] text-slate-300 truncate">
            ...
            · Requested by {{ selectedTicket.requester }}
          </p>
        </div>
      </div>
      ...
      <div class="flex items-center gap-2 shrink-0 self-end sm:self-auto">
        <button
          ...
          class="px-3 py-1.5 rounded-xl bg-white/10 hover:bg-white/20 text-white text-xs font-black transition-colors border border-white/10 cursor-pointer"
        >
          View Scope &amp; Files
        </button>
        <button
          @click="clearSelectedTicket"
          class="px-3 py-1.5 rounded-xl bg-rose-500/20 hover:bg-rose-500/30 text-rose-300 hover:text-white text-xs font-black transition-colors border border-rose-500/30 cursor-pointer"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
      ...
      <div class="lg:col-span-5 space-y-4">
        <div class="bg-white rounded-3xl border border-slate-200/90 shadow-sm p-5 sm:p-6 flex flex-col h-[calc(100vh-230px)] min-h-[580px]">
          <!-- Column Header -->
          <div class="flex items-center justify-between gap-2 pb-4 border-b border-slate-100 shrink-0">
            <div class="flex items-center gap-2.5">
              <div class="w-8 h-8 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center font-black">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
                </svg>
              </div>
              <div>
                <h3 class="text-base font-black text-slate-900 leading-tight">Approved Tickets</h3>
                <p class="text-[11px] text-slate-400 font-medium">Awaiting workforce assignment</p>
              </div>
            </div>
            <div class="flex items-center gap-1.5">
              <span class="px-2.5 py-1 rounded-full bg-slate-100 text-slate-700 text-xs font-black tabular-nums">
                {{ filteredTickets.length }}
              </span>
              <button
                @click="fetchTickets"
                :disabled="loadingTickets"
                class="p-1.5 rounded-xl hover:bg-slate-100 text-slate-400 hover:text-slate-700 transition-colors cursor-pointer"
                title="Refresh approved tickets"
              >
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" :class="{ 'animate-spin': loadingTickets }" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div class="pt-3.5 pb-2 space-y-2.5 shrink-0">
            <div class="relative">
              <input
                v-model="ticketSearch"
                type="text"
                placeholder="Search ticket #, service, location, requester..."
                class="w-full pl-9 pr-4 py-2 rounded-xl border border-slate-200 text-xs font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 bg-slate-50/50"
              />
            ...
            <div class="flex items-center gap-1.5 overflow-x-auto custom-scrollbar pb-1 text-[11px]">
              <button
                ...
                :class="['px-2.5 py-1 rounded-lg font-bold transition-all cursor-pointer whitespace-nowrap', urgencyFilter === 'all' ? 'bg-slate-900 text-white shadow-xs' : 'bg-slate-100 text-slate-500 hover:text-slate-800']"
              >
                All ({{ dispatchQueue.length }})
              </button>
              <button
                @click="urgencyFilter = 'emergency'"
                :class="['px-2.5 py-1 rounded-lg font-bold transition-all cursor-pointer whitespace-nowrap', urgencyFilter === 'emergency' ? 'bg-rose-600 text-white shadow-xs' : 'bg-rose-50 text-rose-700 hover:bg-rose-100']"
              >
                Emergency / High
              </button>
              <button
                @click="urgencyFilter = 'standard'"
                :class="['px-2.5 py-1 rounded-lg font-bold transition-all cursor-pointer whitespace-nowrap', urgencyFilter === 'standard' ? 'bg-slate-900 text-white shadow-xs' : 'bg-slate-100 text-slate-500 hover:text-slate-800']"
              >
                Standard
              </button>
            </div>
          ...
          <div class="flex-1 overflow-y-auto space-y-2.5 pr-1 custom-scrollbar pt-2">
            ...
            <div v-else-if="filteredTickets.length === 0" class="py-16 flex flex-col items-center justify-center text-center p-4">
              ...
              <h4 class="text-sm font-black text-slate-700">No approved tickets pending</h4>
            ...
            >
              ...
              <div class="flex items-center justify-between gap-2 mb-2">
                <div class="flex items-center gap-2">
                  <span class="px-2 py-0.5 rounded-md bg-slate-900 text-white text-[10px] font-black tracking-wider">
                    #{{ ticket.id }}
                  </span>
                  <span class="text-xs font-black text-slate-800 truncate max-w-[160px]">
                ...
                <div class="flex items-center gap-1.5">
                  <span
                    ...
                    class="px-2 py-0.5 rounded-md bg-rose-100 text-rose-700 text-[9px] font-black uppercase tracking-wider"
                  >
                    Emergency
                  </span>
                  <span
                    v-else
                    class="px-2 py-0.5 rounded-md bg-emerald-100 text-emerald-800 text-[9px] font-black uppercase tracking-wider"
                  >
                    Approved
                  </span>
                </div>
              ...
              <!-- Ticket Title -->
              <h4 class="text-xs sm:text-sm font-bold text-slate-900 line-clamp-1 mb-1 group-hover:text-emerald-700 transition-colors">
              ...
              <div class="space-y-0.5 text-[11px] text-slate-500 font-medium">
                ...
                <p class="flex items-center gap-1.5 truncate">
                  ...
                  <span class="truncate">{{ ticket.requester }}</span>
        ...
        <div class="bg-white rounded-3xl border border-slate-200/90 shadow-sm p-5 sm:p-6 flex flex-col h-[calc(100vh-230px)] min-h-[580px]">
          ...
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-100 shrink-0">
            <div class="flex items-center gap-2.5">
              ...
              <div>
                <h3 class="text-base font-black text-slate-900 leading-tight">Unit Workforce</h3>
                <p class="text-[11px] text-slate-400 font-medium">Select a worker to deploy to the active ticket</p>
              </div>
            </div>

            <div class="flex items-center gap-2">
              <span class="text-xs font-bold text-slate-500">Showing:</span>
              <span class="px-2.5 py-1 rounded-full bg-slate-100 text-slate-700 text-xs font-black tabular-nums">
          ...
          <div class="pt-3.5 pb-2 space-y-2.5 shrink-0">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
              ...
              <select
                ...
                class="w-full px-3 py-2 rounded-xl border border-slate-200 text-xs font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 bg-slate-50/50 cursor-pointer"
              >
            ...
            <div class="flex items-center gap-1.5 overflow-x-auto custom-scrollbar pb-1 text-[11px]">
              <button
                ...
                :class="['px-2.5 py-1 rounded-lg font-bold transition-all cursor-pointer whitespace-nowrap', selectedSpecialty === 'all' ? 'bg-slate-900 text-white shadow-xs' : 'bg-slate-100 text-slate-500 hover:text-slate-800']"
              >
                All Specialties
              </button>
              <button
          ...
          <div
            v-if="!selectedTicket"
            class="p-3 bg-amber-50/80 border border-amber-200/80 rounded-2xl flex items-center gap-2.5 text-xs text-amber-800 shrink-0"
          >
          ...
          <div class="flex-1 overflow-y-auto pr-1 custom-scrollbar pt-2">
            <div v-if="filteredWorkers.length === 0" class="py-16 text-center text-slate-400">
              <p class="text-xs font-bold uppercase tracking-wider">No personnel match current filters</p>
              <p class="text-[11px] text-slate-400 mt-1">Try clearing your search or specialty filters.</p>
            </div>
            ...
            <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div
                ...
                :key="worker.id"
                class="p-4 rounded-2xl border border-slate-200/80 bg-white hover:border-slate-300 transition-all shadow-xs flex flex-col justify-between gap-3 group"
              >
                <!-- Worker Info Header -->
                <div>
                  <div class="flex items-start justify-between gap-2 mb-2">
                    <div class="flex items-center gap-2.5 min-w-0">
                      <!-- Avatar -->
                      <div class="w-10 h-10 rounded-2xl bg-gradient-to-br from-slate-100 to-slate-200 text-slate-700 flex items-center justify-center font-black text-xs shrink-0 border border-slate-200/60 shadow-xs">
                        {{ getInitials(worker.name) }}
                      </div>
                      <div class="min-w-0">
                        <h4 class="text-xs sm:text-sm font-black text-slate-900 truncate leading-tight">
                          {{ worker.name }}
                        </h4>
                        <span class="inline-block text-[10px] font-bold text-slate-500 truncate mt-0.5">
                          {{ worker.specialty || worker.role || 'General' }}
                        </span>
                      </div>
                    </div>
                  ...
                  <div class="text-[11px] text-slate-500 space-y-1 bg-slate-50/70 p-2.5 rounded-xl border border-slate-100">
                    <p class="flex items-center justify-between">
                      <span class="text-slate-400 font-bold">Contact:</span>
                      <span class="font-bold text-slate-700">{{ worker.contact_number || 'No contact provided' }}</span>
                    </p>
                    <p class="flex items-center justify-between">
                      <span class="text-slate-400 font-bold">Active Load:</span>
                      <span v-if="worker.status === 'Working' || worker.assignedTicket" class="font-black text-amber-700 truncate max-w-[140px]">
                ...
                <div>
                  <button
                    ...
                    :class="[
                      'w-full py-2 px-3 rounded-xl text-xs font-black transition-all flex items-center justify-center gap-1.5 cursor-pointer shadow-xs active:scale-98',
                  ...
                  <div
                    v-else
                    class="py-2 px-3 rounded-xl text-center text-[11px] font-bold text-slate-400 bg-slate-50 border border-dashed border-slate-200"
                  >
    ...
    >
      <div class="bg-white rounded-3xl sm:rounded-[2rem] w-full max-w-lg p-6 sm:p-8 shadow-2xl border border-slate-100 animate-scale-up">
        <!-- Modal Header -->
        <div class="flex items-center justify-between pb-4 border-b border-slate-100 mb-5">
          <div>
            ...
            <h3 class="text-xl font-black text-slate-900 mt-1">Assign Worker to Ticket</h3>
          </div>
          <button
        ...
        <div class="grid grid-cols-2 gap-3 p-4 rounded-2xl bg-slate-50 border border-slate-100 mb-5 text-xs">
          <div>
            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-0.5">Ticket #{{ selectedTicket.id }}</span>
            <p class="font-bold text-slate-900 truncate">{{ selectedTicket.title || selectedTicket.type }}</p>
            <p class="text-[11px] text-slate-500 truncate">{{ selectedTicket.location || selectedTicket.college_building }}</p>
          </div>
          <div>
            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-0.5">Assigned Technician</span>
            <p class="font-black text-emerald-700 truncate">{{ selectedWorkerForAssign.name }}</p>
            <p class="text-[11px] text-slate-500 truncate">{{ selectedWorkerForAssign.specialty || 'General' }}</p>
          </div>
        </div>
        ...
        <form @submit.prevent="submitDispatchAssignment" class="space-y-4">
          ...
          <div>
            <label class="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-1.5">
          ...
          <div>
            ...
            <div class="grid grid-cols-3 gap-2 mb-2">
              <button
                ...
                :class="['py-2 px-2 rounded-xl text-xs font-bold border transition-all cursor-pointer text-center', assignForm.workingDays === 3 ? 'bg-emerald-50 border-emerald-500 text-emerald-800 font-black shadow-xs' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50']"
              >
                Simple (3 days)
              </button>
              <button
                ...
                :class="['py-2 px-2 rounded-xl text-xs font-bold border transition-all cursor-pointer text-center', assignForm.workingDays === 7 ? 'bg-emerald-50 border-emerald-500 text-emerald-800 font-black shadow-xs' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50']"
              >
                Moderate (7 days)
              </button>
              <button
                ...
                :class="['py-2 px-2 rounded-xl text-xs font-bold border transition-all cursor-pointer text-center', assignForm.workingDays === 21 ? 'bg-emerald-50 border-emerald-500 text-emerald-800 font-black shadow-xs' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50']"
              >
                Complex (21 days)
              </button>
            </div>
            <div class="relative">
              <input
                ...
                max="90"
                required
                class="w-full px-4 py-2 rounded-xl border border-slate-200 text-xs font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500"
              />
              <span class="absolute right-3.5 top-2 text-xs font-bold text-slate-400">working days</span>
            </div>
          </div>

          <!-- Emergency Priority Checkbox -->
          <div class="flex items-center gap-2.5 p-3 rounded-xl bg-slate-50 border border-slate-100">
            <input
            ...
            />
            <label for="emergency-dispatch-toggle" class="text-xs font-bold text-slate-700 cursor-pointer select-none">
          ...
          <div>
            ...
            <textarea
              ...
              class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-xs font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500 resize-none"
            ></textarea>
          </div>
          ...
          <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100 mt-6">
            <button
              type="button"
              @click="closeAssignmentModal"
              class="px-5 py-2.5 rounded-xl border border-slate-200 text-xs font-black text-slate-600 hover:bg-slate-50 cursor-pointer"
            >
              Cancel
            </button>
            <button
              ...
              class="px-6 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-black transition-all shadow-sm shadow-emerald-200 active:scale-95 disabled:opacity-50 cursor-pointer flex items-center gap-2"
            >
              <svg v-if="submittingAssignment" class="animate-spin h-3.5 w-3.5 text-white" fill="none" viewBox="0 0 24 24">
      ...
      <div class="bg-white rounded-3xl sm:rounded-[2.5rem] w-full max-w-2xl shadow-2xl border border-slate-100 overflow-hidden animate-scale-up max-h-[85vh] flex flex-col">
        ...
        <div class="p-6 sm:p-8 space-y-5 overflow-y-auto flex-1 custom-scrollbar">
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="p-4 bg-slate-50 rounded-2xl border border-slate-100">
              ...
              <p class="text-xs font-bold text-slate-900">{{ modalTicket.location || modalTicket.college_building || 'Campus Facility' }}</p>
            </div>
            <div class="p-4 bg-slate-50 rounded-2xl border border-slate-100">
              <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Office / Room</span>
              <p class="text-xs font-bold text-slate-900">{{ modalTicket.office_room || 'N/A' }}</p>
            </div>
            <div class="p-4 bg-slate-50 rounded-2xl border border-slate-100">
              <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Requester Name</span>
              <p class="text-xs font-bold text-slate-900">{{ modalTicket.requester }}</p>
            </div>
            <div class="p-4 bg-slate-50 rounded-2xl border border-slate-100">
              <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Contact Number</span>
              <p class="text-xs font-bold text-slate-900">{{ modalTicket.contact_number || 'N/A' }}</p>
            </div>
          </div>

          <div>
            <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1.5">Scope / Particulars</span>
            <div class="p-4 bg-slate-50 rounded-2xl border border-slate-100 text-xs font-medium text-slate-700 leading-relaxed italic">
          ...
          <div v-if="modalTicket.attachments && modalTicket.attachments.length > 0">
            <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Attached Files &amp; Proof</span>
        ...
        <div class="p-4 bg-slate-50 border-t border-slate-100 flex justify-between items-center shrink-0">
          <button
            ...
            class="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs uppercase tracking-wider transition-all cursor-pointer"
          >
          ...
          <button
            @click="showDetailsModal = false"
            class="px-5 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-black text-xs uppercase tracking-wider transition-all cursor-pointer"
          >
            Close
          </button>
        </div>
