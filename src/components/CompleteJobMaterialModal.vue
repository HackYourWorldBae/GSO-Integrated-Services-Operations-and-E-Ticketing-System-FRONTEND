<template>
  <Teleport to="body">
    <Transition name="modal">
      <div
        v-if="isOpen"
        class="fixed inset-0 z-[150] flex items-center justify-center p-3 sm:p-6 bg-slate-950/70 backdrop-blur-md overflow-y-auto pointer-events-auto"
        @click.self="emitClose"
      >
        <div class="my-auto bg-white w-full max-w-3xl max-h-[calc(100dvh-2rem)] sm:max-h-[88vh] rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden border border-slate-200 animate-scale-up pointer-events-auto">
          ...
          <div class="flex-1 overflow-y-auto p-6 sm:p-8 space-y-6 custom-scrollbar">
            ...
            <div v-else class="py-6 px-4 bg-emerald-50/50 border border-dashed border-emerald-300 rounded-2xl text-center space-y-1.5">
              <div class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-100 text-emerald-800 text-xs font-black">
                <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                <span>Labor Only Service (Initial Assessment Preserved)</span>
              </div>
              <p class="text-xs font-bold text-slate-700">No billed materials or spare parts are attached to this ticket.</p>
              <p class="text-[11px] text-slate-500">The ticket will resolve and close directly without requiring material liquidation.</p>
            </div>
            ...
            <div class="space-y-1.5">
              ...
              <textarea
                ...
                rows="2"
                placeholder="Add any specific service completion notes, parts warranty, or observations..."
...
watch(() => props.isOpen, (newVal) => {
  if (newVal) {
    ...
    if (props.ticket?.is_labor_only) {
      noMaterialsUsed.value = true;
      materials.value = [
        { material_name: '', quantity: 1, unit_measurement: 'pcs', unit_price: 0 },
      ];
    } else if (Array.isArray(props.ticket?.materials) && props.ticket.materials.length > 0) {
...
const submitCompletion = async () => {
  ...
  try {
    const payload = {
      is_labor_only: noMaterialsUsed.value ? 1 : 0,
    ...
    toast.success(`Ticket #${props.ticket.id} completed. Awaiting requestor rating to close.`);
    emit('completed', {
