      ...
      <div class="space-y-6 animate-fade-in relative pb-12">
        ...
        <div class="bg-white p-4 sm:p-6 rounded-2xl sm:rounded-[2rem] border border-slate-200 shadow-sm flex flex-col md:flex-row gap-4 items-center">
          <div class="flex-1 w-full relative">
            ...
            <input v-model="searchQuery" type="text" placeholder="Search Full Ticket Number (e.g. SSU-TIC-10-2026)" class="w-full pl-12 pr-4 py-3 min-h-[44px] bg-slate-50 border border-slate-200 rounded-2xl text-base sm:text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all" />
          </div>
        ...
        <div v-if="showDetailsModal && selectedTicket" class="fixed inset-0 z-[150] flex items-center justify-center p-3 sm:p-6 bg-slate-950/70 backdrop-blur-md animate-fade-in pointer-events-auto overflow-y-auto" @click.self="closeDetailsModal">
          ...
          <div class="space-y-6">
            ...
            <div class="grid grid-cols-2 gap-6 bg-slate-50 p-6 rounded-2xl border border-slate-100">
              ...
              <div>
                <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Reporter Contact</p>
                <a
                  v-if="(selectedTicket.contact_number || selectedTicket.contact) && (selectedTicket.contact_number || selectedTicket.contact) !== 'N/A'"
                  :href="`tel:${selectedTicket.contact_number || selectedTicket.contact}`"
                ...
                >
                  ...
                  <span class="text-sm sm:text-base font-black font-mono tracking-wide text-emerald-800 group-hover:text-emerald-900">{{ selectedTicket.contact_number || selectedTicket.contact }}</span>
                </a>
  ...
  try {
    ...
    if (response.data?.data?.tickets) {
      tickets.value = response.data.data.tickets.map(t => ({
        ...
        requestedBy: t.requester || t.requestedBy || t.requested_by || t.details?.reported_by || t.details?.end_user || (t.user ? `${t.user.first_name || ''} ${t.user.last_name || ''}`.trim() : '') || (t.first_name || t.last_name ? `${t.first_name || ''} ${t.last_name || ''}`.trim() : '') || 'End User',
