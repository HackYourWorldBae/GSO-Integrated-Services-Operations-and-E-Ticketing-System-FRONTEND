    ...
    <template #header-title>
      <div class="flex flex-col">
        <div class="flex items-center gap-2">
          ...
          <h2 class="text-xl font-bold text-slate-900 tracking-tight leading-none">LEAU Scheduled &amp; Dispatched</h2>
          ...
          <div class="relative bg-gradient-to-br from-amber-900 to-amber-800 px-6 sm:px-10 py-7 overflow-hidden">
            ...
            <div class="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <div class="flex items-center self-start sm:self-auto gap-3">
                ...
                <router-link
                  to="/admin/leau/active-tickets"
                  class="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-2xl text-xs font-black uppercase tracking-wider transition-all shadow-sm active:scale-95 flex items-center gap-1.5"
                >
          ...
          <div class="bg-white px-4 sm:px-6 pb-6">
            ...
            <div v-else class="divide-y divide-slate-100">
              <div
                ...
                :key="ticket.id"
                :id="'ticket-' + ticket.id"
                :class="['group flex flex-col lg:flex-row lg:items-center justify-between gap-4 py-4 px-4 sm:px-6 hover:bg-slate-50/80 border-b border-slate-100 last:border-b-0 rounded-2xl transition-all duration-200 cursor-default', highlightedTicketId === ticket.id ? 'ring-2 ring-amber-500 bg-amber-50/60 shadow-md' : '']"
              >
...
const performStartEarly = async (ticketId) => {
  ...
  try {
    ...
    await fetchTickets();
    router.push('/admin/leau/active-tickets');
  } catch (error) {
    console.error(error);
