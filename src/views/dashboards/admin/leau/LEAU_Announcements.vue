      ...
      <div class="space-y-6 animate-fade-in relative pb-12 px-4 sm:px-8 py-8 max-w-[1600px] mx-auto min-h-screen">
        <div class="flex justify-between items-end mb-6">
          <div class="max-w-2xl">
            ...
            <p class="text-sm text-slate-500 font-medium">Manage and publish scheduled equipment or facility projects to the public dashboard.</p>
          </div>
            ...
            <div class="p-6 overflow-y-auto">
              <form @submit.prevent="submitProject" class="space-y-4">
                <div>
                  ...
                  <input v-model="form.title" type="text" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all" placeholder="E.g., Aircon Installation at Library">
                </div>
                ...
                <div>
                  ...
                  <input v-model="form.manpower" type="text" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all" placeholder="E.g., 2 Technicians">
                </div>
        ...
        <!-- Update Modal -->
        <div v-if="showUpdateModal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div class="bg-white rounded-3xl shadow-2xl max-w-lg w-full overflow-hidden flex flex-col max-h-[90vh]">
            <div class="px-6 py-5 border-b border-slate-100 flex justify-between items-center shrink-0">
    ...
    const fetchProjects = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          projects.value = (data.data?.projects || []).filter(p => p.unit_id === 2); 
        }
      } catch (err) {
