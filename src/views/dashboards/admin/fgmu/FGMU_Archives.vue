  ...
  <MainLayout>
    ...
    <template #header-title>
      <div class="flex flex-col">
        ...
        <p class="text-[10px] text-emerald-600 font-extrabold tracking-[0.2em] uppercase">FGMU Archiving</p>
      </div>
    </template>
        ...
        <div class="bg-white rounded-[2rem] p-8 max-w-2xl w-full shadow-2xl transform transition-all max-h-[90vh] overflow-y-auto custom-scrollbar">
          ...
          <div class="space-y-6">
            ...
            <div class="grid grid-cols-2 gap-6 bg-slate-50 p-6 rounded-2xl border border-slate-100">
              ...
              <div>
                <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Requester Contact</p>
                <a
...
const openJobRequestForm = async (ticket) => {
  try {
    ...
    // Check if DOCX or PDF attachment exists in ticket attachments
...
const fetchArchives = async () => {
  try {
    const response = await api.get('tickets/archives/FGMU');
    if (response.data?.data?.tickets) {
      tickets.value = response.data.data.tickets.map(t => ({
        ...
        requester: t.requester || t.requestedBy || t.requested_by || (t.user ? `${t.user.first_name || ''} ${t.user.last_name || ''}`.trim() : '') || 'End User',
        requestedBy: t.requester || t.requestedBy || t.requested_by || t.details?.end_user || (t.user ? `${t.user.first_name || ''} ${t.user.last_name || ''}`.trim() : '') || (t.first_name || t.last_name ? `${t.first_name || ''} ${t.last_name || ''}`.trim() : '') || 'End User',
