  ...
  <MainLayout>
    ...
    <template #header-title>
      <div class="flex flex-col">
        ...
        <p class="text-[10px] text-emerald-600 font-extrabold tracking-[0.2em] uppercase">Personnel Management & Worker Assignment</p>
      </div>
    </template>
    ...
    <template #main-content>
      <div class="space-y-6 animate-fade-in pb-12 px-4 sm:px-8 py-8 max-w-[1600px] mx-auto min-h-screen">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-2">
          <div>
            <h2 class="text-3xl font-black tracking-tight text-slate-900 mb-1">FGMU Personnel & Operations</h2>
            <p class="text-sm text-slate-500 font-medium">Manage personnel availability, categories, and assign approved tickets to staff.</p>
          </div>
        ...
        <div v-if="selectedTicket" class="p-6 sm:p-8 rounded-3xl sm:rounded-[2.5rem] bg-gradient-to-br from-slate-900 via-slate-850 to-slate-900 text-white shadow-xl relative overflow-hidden border border-emerald-500/30 animate-scale-up">
          ...
          <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-white/10">
            <div>
              ...
              <h3 class="text-2xl font-black text-white tracking-tight">{{ selectedTicket.type || selectedTicket.title || 'Facilities Service Request' }}</h3>
          ...
          <div class="relative z-10 grid grid-cols-1 lg:grid-cols-3 gap-6 pt-6 items-end">
            <!-- Implementation Date -->
            <div>
              <label for="impl-date-input" class="text-[10px] font-black text-slate-300 uppercase tracking-widest block mb-2">
            ...
            <div>
              <div class="flex items-center justify-between mb-2">
                <label for="working-days-input" class="text-[10px] font-black text-slate-300 uppercase tracking-widest">
            ...
            <div class="flex items-center gap-3 p-3 bg-white/5 border border-white/10 rounded-2xl">
              ...
              />
              <label for="emergency-checkbox" class="text-xs font-bold text-slate-200 cursor-pointer select-none">
        ...
        <div class="bg-white rounded-3xl sm:rounded-[2rem] w-full max-w-md p-6 sm:p-8 shadow-2xl border border-slate-100 animate-scale-up">
          ...
          <form @submit.prevent="submitAddPersonnel" class="space-y-4">
            <div>
              ...
              <input v-model="form.firstName" required placeholder="e.g. Juan" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
            </div>
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Middle Initial</label>
                <input v-model="form.middleInitial" maxlength="2" placeholder="e.g. D." class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
              </div>
              <div>
            ...
            <div>
              ...
              <input v-model="form.lastName" required placeholder="e.g. Dela Cruz" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
            </div>
            <div>
        ...
        <div class="bg-white rounded-3xl sm:rounded-[2rem] w-full max-w-2xl p-6 sm:p-8 shadow-2xl border border-slate-100 animate-scale-up flex flex-col max-h-[85vh]">
          <div class="flex items-center justify-between mb-4">
            <div>
              ...
              <p class="text-xs text-slate-500 font-medium">Select a ticket approved by the Director to assign personnel.</p>
            </div>
          ...
          <div class="p-6 sm:p-8 space-y-6 overflow-y-auto flex-1 custom-scrollbar">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div class="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                ...
                <p class="text-xs font-bold text-slate-900">{{ reviewTicket.location || reviewTicket.college_building || 'Campus Facility' }}</p>
              </div>
              <div class="p-4 bg-slate-50 rounded-2xl border border-slate-100">
...
const assignWorkerToCurrentTicket = (worker) => {
  ...
  const taskDesc = selectedTicket.value.type || 'Facilities Maintenance Work';
...
const dispatchAll = async () => {
  ...
  try {
    for (const assign of currentAssignments.value) {
      ...
      if (worker && (worker.status === 'Available' || worker.status === 'Working' || worker.assignedTicket)) {
        await api.post('dispatch/assign', {
          ...
          task_notes: isProjectIdentifier(selectedTicket.value) ? (selectedTicket.value.type || 'Facilities Project Work') : (selectedTicket.value.type || 'Facilities Maintenance Task'),
    ...
    clearSelectedTicket();
    router.push('/admin/fgmu/dispatched');
  } catch (error) {
