  ...
  <MainLayout>
    ...
    <template #header-title>
      <div class="flex flex-col">
        ...
        <p class="text-[10px] text-emerald-600 font-extrabold tracking-[0.2em] uppercase mt-1">FGMU Admin Approval Queue</p>
      </div>
    </template>
                ...
                <tr v-else-if="paginatedTickets.length === 0">
                  <td colspan="6" class="py-16 text-center">
                    <div class="max-w-sm mx-auto flex flex-col items-center">
                      ...
                      <p class="text-xs text-slate-400 mt-1">All service requests have been reviewed, approved, or processed.</p>
                    </div>
                  </td>
                </tr>
          ...
          <div class="px-6 py-4 bg-slate-50/50 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div class="flex items-center gap-3">
              ...
              <select
                v-model="perPage"
                @change="currentPage = 1"
                class="px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-700 text-xs font-semibold focus:outline-none focus:border-emerald-500"
              >
          ...
          >
            ...
            <div class="grid grid-cols-2 gap-2 text-xs p-3 rounded-2xl bg-slate-50 border border-slate-100">
              ...
              <div v-if="ticket.office_room" class="col-span-2 pt-1 border-t border-slate-200/60">
                <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Office / Room</span>
                <span class="font-semibold text-slate-700">{{ ticket.office_room }}</span>
              </div>
            </div>

            <!-- Attachments Link if any -->
            <div v-if="ticket.attachments && ticket.attachments.length > 0" class="flex items-center gap-1.5 text-xs text-emerald-600 font-bold">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" />
              </svg>
        ...
        <div class="bg-white rounded-3xl border border-slate-200 shadow-2xl max-w-xl w-full p-6 animate-scale-up space-y-5 max-h-[90vh] overflow-y-auto custom-scrollbar">
          ...
          <div class="space-y-4 text-xs">
            ...
            <div class="grid grid-cols-2 gap-3 p-4 rounded-2xl bg-slate-50 border border-slate-100">
              <div>
                <span class="text-[10px] font-black uppercase tracking-wider text-slate-400 block mb-0.5">Location / Building</span>
                <span class="font-bold text-slate-800">{{ selectedTicketForModal.location || 'Main Campus' }}</span>
              </div>
              <div>
                <span class="text-[10px] font-black uppercase tracking-wider text-slate-400 block mb-0.5">Office / Room</span>
                <span class="font-bold text-slate-800">{{ selectedTicketForModal.office_room || 'N/A' }}</span>
              </div>
            </div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                ...
                >
                  <div class="flex items-center gap-2 truncate">
                    ...
                    <span class="text-xs font-semibold text-slate-700 group-hover:text-emerald-800 truncate">
