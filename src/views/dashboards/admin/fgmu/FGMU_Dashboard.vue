          ...
          <div class="flex flex-wrap items-center justify-between gap-4 border-b border-slate-100 pb-5">
            <div>
              <div class="flex items-center gap-2 mb-1">
                ...
                <span v-if="activeProjects.length > 0" class="px-2.5 py-0.5 rounded-full bg-emerald-100 text-emerald-800 text-xs font-black">
                  {{ activeProjects.length }} Active
                </span>
              </div>
              <p class="text-xs text-slate-500 font-medium">Scheduled maintenance, infrastructure, and facility developments posted for FGMU.</p>
            </div>
            <router-link to="/admin/fgmu/announcements" class="inline-flex items-center gap-2 bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold px-4 py-2.5 rounded-xl transition-all shadow-sm">
          ...
          <div v-else-if="activeProjects.length === 0" class="flex flex-col sm:flex-row items-center justify-between gap-4 p-6 bg-slate-50 rounded-2xl border border-slate-200/70">
            <div class="flex items-center gap-3.5">
              ...
              <div>
                ...
                <p class="text-xs text-slate-500">Publish scheduled campus facilities projects to notify university stakeholders.</p>
              </div>
            </div>
            <router-link to="/admin/fgmu/announcements" class="text-xs font-bold text-emerald-700 hover:text-emerald-800 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 px-4 py-2 rounded-xl transition-all flex items-center gap-1.5 shrink-0">
            ...
            <div v-for="project in activeProjects" :key="project.id" class="bg-gradient-to-b from-white to-slate-50 border border-slate-200/80 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all flex flex-col relative overflow-hidden">
              ...
              <div class="flex justify-between items-center mb-3">
                <div class="flex items-center gap-2">
                  ...
                  <span class="px-2.5 py-0.5 bg-emerald-50 text-emerald-700 text-[10px] font-extrabold rounded-full border border-emerald-100 flex items-center gap-1">
              ...
              <p class="text-xs text-slate-600 line-clamp-2 leading-relaxed mb-4">
                {{ project.description || 'Scheduled facility maintenance and development.' }}
              </p>
              ...
              <div class="mt-auto pt-2 border-t border-slate-100 flex justify-end">
                <router-link to="/admin/fgmu/announcements" class="text-xs font-bold text-emerald-700 hover:text-emerald-900 flex items-center gap-1">
...
const fetchActiveProjects = async () => {
  ...
  try {
    ...
    if (res.data?.data?.projects) {
      ...
      activeProjects.value = res.data.data.projects.filter(p => Number(p.unit_id) === 1);
    }
  } catch (err) {
    console.error('Failed to load active FGMU projects:', err);
  } finally {
