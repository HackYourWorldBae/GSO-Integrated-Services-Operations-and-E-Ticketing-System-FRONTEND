  ...
  <MainLayout>
    <template #sidebar-links>
      ...
      <template v-else>
        ...
        <router-link to="/director/fgmu/queues" class="nav-item">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <tbody class="divide-y divide-slate-100">
                ...
                >
                  ...
                  <td class="px-3 py-3 whitespace-nowrap text-right" @click.stop>
                    <!-- Pending Tab Actions -->
                    <div v-if="activeTab === 'pending'" class="flex items-center justify-end gap-1.5">
                      <button
                        @click="openDetailsModal(ticket)"
      ...
      <div class="bg-white rounded-3xl max-w-3xl w-full shadow-2xl border border-slate-200 animate-scale-up flex flex-col max-h-[calc(100dvh-4rem)] sm:max-h-[calc(100dvh-5rem)] overflow-hidden">
        ...
        <div class="p-4 sm:p-5 bg-slate-50 border-t border-slate-100 flex flex-wrap items-center justify-between gap-3 shrink-0">
          ...
          <div v-if="activeTab === 'pending'" class="flex items-center gap-2">
            ...
            <button @click="initiateApproval(selectedTicketForModal); closeDetailsModal()" class="px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-black uppercase tracking-wider shadow-md shadow-emerald-600/20 transition-all cursor-pointer">
              Approve Request
            </button>
          </div>
