          ...
          <div class="p-4 sm:p-8 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-50/50">
            <div class="flex items-center gap-4">
              <div class="relative group cursor-pointer shrink-0" @click="triggerAvatarUpload" title="Click to change profile picture">
                ...
                />
                <div class="w-16 h-16 rounded-[1.25rem] overflow-hidden bg-slate-900 flex items-center justify-center text-white shadow-lg shadow-slate-900/20 border-2 border-emerald-500/40">
                  <img v-if="avatarPreviewUrl" :src="avatarPreviewUrl" alt="Avatar" class="w-full h-full object-cover" />
                  <span v-else class="text-2xl font-black">{{ form.firstName ? form.firstName.charAt(0).toUpperCase() : 'D' }}</span>
                </div>
              ...
              <div>
                <div class="flex items-center gap-2 flex-wrap">
                  <h3 class="text-lg font-black text-slate-900 tracking-tight">Executive Profile</h3>
                ...
                <p class="text-xs text-slate-500 font-medium mt-0.5">Manage your executive profile details and authentication credentials</p>
              </div>
            </div>
          ...
          <div class="p-4 sm:p-8 md:p-12 space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
              ...
              <div class="space-y-2.5 md:col-span-2">
                ...
                <p class="text-[11px] text-slate-400 font-medium ml-1">Official university director email address. Used for login and system notifications.</p>
              </div>
            ...
            <div class="p-4 sm:p-5 rounded-2xl border flex items-start gap-3.5 transition-colors" :class="form.emailNotificationsEnabled ? 'bg-emerald-50/60 border-emerald-200/70' : 'bg-slate-50 border-slate-200'">
              ...
              <div class="flex-1 min-w-0 space-y-1">
                ...
                <p class="text-xs leading-relaxed font-medium" :class="form.emailNotificationsEnabled ? 'text-emerald-800/80' : 'text-slate-500'">
                  When on, new requests and SSU incident reports across all sub-units notify <span class="font-bold">this address ({{ form.email || 'your email' }})</span>. Director-level alerts cover FGMU, LEAU, and SSU. Toggle off to stay dashboard-only.
                </p>
                <p v-if="!form.emailNotificationsEnabled" class="text-[11px] font-bold text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-2.5 py-1.5">Email alerts are paused — SSU incident and ticket update emails are muted until re-enabled.</p>
              </div>
            </div>
            ...
            <div class="p-4 sm:p-5 rounded-2xl bg-emerald-50/60 border border-emerald-200/70 flex items-start gap-3.5">
              ...
              <div class="space-y-0.5">
                ...
                <p class="text-xs text-emerald-800 leading-relaxed font-medium">
                  Your account holds institutional oversight across all General Services Office sub-units (FGMU, LEAU, SSU) with university-wide report generation and material logs and reporting authorizations.
                </p>
              </div>
            </div>
          </div>
...
const handleSave = async () => {
  ...
  try {
    ...
    if (result.success) {
      toast.success('Director profile updated successfully!');
    } else {
