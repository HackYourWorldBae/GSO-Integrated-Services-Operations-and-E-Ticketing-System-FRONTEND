  ...
  <MainLayout>
    ...
    <template #header-title>
      <div class="flex flex-col">
        ...
        <p class="text-[10px] text-emerald-600 font-extrabold tracking-[0.2em] uppercase">Landscaping & Environment Control</p>
      </div>
    </template>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <div class="p-8 rounded-[2rem] bg-white border border-slate-200 shadow-sm">
            <div class="flex items-center justify-between mb-6">
              ...
              <span class="text-[10px] font-black text-amber-500/40 uppercase tracking-widest">New</span>
            </div>
            <h3 class="text-4xl font-black text-slate-900 tabular-nums">{{ unplannedCount }}</h3>
            <p class="text-sm text-slate-500 font-bold uppercase tracking-wider">Unplanned Requests</p>
          </div>
                ...
                <tr v-for="ticket in regularTickets" :key="ticket.id" class="group hover:bg-slate-50/50 transition-colors">
                  ...
                  <td class="py-6 px-4">
                    <span class="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest"
                      :class="ticket.status === 'Urgent' ? 'bg-rose-50 text-rose-600' : 'bg-emerald-50 text-emerald-600'">
                      {{ ticket.status }}
                    </span>
                  </td>
            ...
            <div v-for="ticket in regularTickets" :key="'mob-req-' + ticket.id" class="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-3 shadow-xs">
              <div class="flex items-center justify-between">
                ...
                <span class="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md border"
                  :class="ticket.status === 'Urgent' ? 'bg-rose-50 text-rose-600 border-rose-200' : 'bg-emerald-50 text-emerald-700 border-emerald-200'">
              ...
              <div class="flex items-center gap-2 pt-2 border-t border-slate-200/80">
                ...
                <router-link :to="'/dispatcher/leau/workers?ticket=' + ticket.id" class="flex-1 py-2 rounded-xl bg-emerald-600 text-white text-xs font-bold hover:bg-emerald-700 transition-colors text-center shadow-xs">
                  Assign
                </router-link>
              </div>
            </div>
    ...
    <template #modal-overlay>
      ...
      <div v-if="showTicketModal" class="absolute inset-0 z-[60] overflow-y-auto custom-scrollbar pointer-events-auto bg-slate-900/60 backdrop-blur-sm animate-fade-in">
        ...
        <div class="bg-[#F8FAFC] w-full max-w-4xl h-fit rounded-[3rem] shadow-2xl overflow-hidden relative z-10 animate-scale-up border border-slate-200">
          <template v-if="isProjectIdentifier(selectedTicket)">
            ...
            <div class="p-10 space-y-8">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-2">
                  ...
                  <p class="text-sm font-bold text-slate-900 bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">{{ selectedTicket.type }}</p>
                </div>
                <div class="space-y-2">
                  <label class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Location</label>
                  <p class="text-sm font-bold text-slate-900 bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">{{ selectedTicket.location }}</p>
                </div>
              </div>
              <div class="space-y-3">
          ...
          <div class="bg-slate-900 p-8 text-white flex justify-between items-end border-b-4 border-emerald-500">
            <div>
              ...
              <h3 class="text-3xl font-black tracking-tighter">Landscaping Request <span class="text-emerald-500">({{ selectedTicket.id }})</span></h3>
            ...
            <div class="text-right">
              ...
              <p class="text-2xl font-black text-emerald-500">{{ selectedTicket.jr_no }}</p>
            </div>
            <button @click="showTicketModal = false" class="absolute top-6 right-8 text-slate-400 hover:text-white transition-colors">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
...
const fetchDispatchQueue = async () => {
  try {
    const response = await api.get('tickets/dispatch/LEAU');
