          ...
          <div class="overflow-x-auto">
            <table class="w-full text-left min-w-[750px]">
              ...
              <tbody class="divide-y divide-slate-50">
                <tr v-for="ticket in mockTickets" :key="ticket.id" class="group hover:bg-slate-50/50 transition-colors">
                  <td class="py-6 px-4">
...
const unplannedCount = computed(() => mockTickets.value.length);
const activeGardenersCount = computed(() => ticketsStore.activeTicketsByUnit('LEAU').length);
const beautifiedZonesCount = computed(() => ticketsStore.completedTicketsByUnit('LEAU').length);
...
const mockTickets = computed(() => ticketsStore.dispatchedOrScheduledTickets('LEAU').map(t => ({
  ...t,
  id: t.ticketId || t.id,
  type: t.service || 'Landscaping',
  requester: t.requestedBy || 'Requester',
  location: t.location || 'Campus',
  status: 'Urgent',
  college_building: t.location || 'Main Building',
  office_room: t.office_room || 'Garden',
