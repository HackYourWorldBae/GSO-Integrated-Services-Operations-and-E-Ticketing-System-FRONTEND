<template>
  <MainLayout>
    <template #sidebar-links>
      ...
      <router-link to="/dispatcher/fgmu/dispatched" class="nav-item active">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <tbody>
                ...
                <tr v-for="ticket in activeTickets" :key="ticket.id" class="group transition-all duration-200">
                  ...
                  <td class="py-6 px-6 bg-slate-50/60 border-y border-r border-slate-200 rounded-r-2xl group-hover:bg-white group-hover:border-emerald-500 transition-all text-right">
                    <span class="px-3 py-1 bg-emerald-100 text-emerald-700 text-[10px] font-bold uppercase tracking-widest rounded-full">In Progress</span>
                  </td>
                </tr>
              </tbody>
            </table>
...
const startJobEarly = async (ticketId) => {
  const result = await Swal.fire({
    ...
    text: 'This will move the ticket to Actively Performed regardless of the implementation date.',
    icon: 'warning',
  ...
  if (result.isConfirmed) {
    ...
    try {
      ...
      console.error(error);
      Swal.fire('Error', error.response?.data?.message || 'Failed to start job early.', 'error');
    } finally {
      loading.value = false;
    }
  }
