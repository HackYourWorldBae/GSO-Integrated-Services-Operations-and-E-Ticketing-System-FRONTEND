      ...
      <div class="space-y-5 sm:space-y-8 animate-fade-in">
        ...
        <div v-else-if="!isUserVerified" class="rounded-2xl border border-amber-200/90 bg-gradient-to-r from-amber-50/90 via-orange-50/80 to-amber-50/90 p-4 sm:p-6 shadow-sm">
          <div class="flex items-start gap-3.5">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 mb-1.5 flex-wrap">
                <h4 class="text-sm sm:text-base font-black text-amber-950">Institutional ID Verification Under Review</h4>
                <span class="px-2.5 py-0.5 text-xs font-black rounded-full bg-amber-200 text-amber-900">Pending Approval</span>
              </div>
              <p class="text-xs sm:text-sm text-amber-950 font-medium max-w-2xl leading-relaxed">
        ...
        <!-- Hero CTA Banner -->
        <div class="relative overflow-hidden rounded-2xl sm:rounded-3xl bg-gradient-to-br from-emerald-700 via-emerald-600 to-emerald-800 p-5 sm:p-8 shadow-xl">
          ...
          <div class="relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5 sm:gap-6">
            <div class="max-w-xl">
              <p class="text-emerald-200 text-xs sm:text-sm font-black uppercase tracking-wider mb-1">Quick Action</p>
              <h3 class="text-white text-xl sm:text-3xl font-black leading-tight">Need campus assistance?</h3>
            ...
            >
              <svg class="h-4 w-4 text-amber-300 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
          ...
          <div class="px-4 sm:px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <div class="flex items-center gap-2.5">
              <div class="w-1.5 h-5 sm:h-6 rounded-full bg-emerald-500"></div>
              <h3 class="text-sm sm:text-lg font-black text-slate-900">Ticket Status Updates</h3>
            </div>
            <router-link to="/user/tickets" class="min-h-[40px] px-3 py-1.5 rounded-lg text-xs sm:text-sm font-bold text-emerald-700 hover:text-emerald-800 hover:bg-emerald-50/60 active:bg-emerald-100/60 transition-colors inline-flex items-center gap-1.5">
              <span>View All</span>
          ...
          <!-- Empty state -->
          <div v-if="recentUpdates.length === 0" class="py-14 sm:py-16 flex flex-col items-center text-center px-6">
            <div class="w-14 h-14 sm:w-16 sm:h-16 bg-slate-100 rounded-2xl flex items-center justify-center mb-3 sm:mb-4">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7 sm:h-8 sm:w-8 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
              </svg>
            </div>
            <p class="text-sm sm:text-base font-black text-slate-700 mb-1">No recent activity</p>
            <p class="text-xs sm:text-sm text-slate-500 font-medium">Your ticket updates will appear here.</p>
          </div>
          ...
          <div v-else class="divide-y divide-slate-100">
            <div
              ...
              class="group flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 p-4 sm:px-6 sm:py-4.5 hover:bg-slate-50/80 active:bg-slate-100/70 transition-colors cursor-pointer"
            >
              <div class="flex items-start gap-3 sm:gap-3.5 min-w-0 flex-1">
                <!-- Status indicator icon -->
                <div class="flex-shrink-0 mt-0.5">
                  <div class="relative w-10 h-10 sm:w-11 sm:h-11 rounded-xl flex items-center justify-center" :class="getUpdateIconBg(update.status)">
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-center justify-between sm:justify-start gap-2 flex-wrap">
                    <p class="text-sm sm:text-base font-black text-slate-900 group-hover:text-emerald-700 transition-colors truncate">
                      {{ update.service || update.title }}
                    </p>
                    <span class="inline-flex items-center gap-1.5 px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full text-[11px] sm:text-xs font-black uppercase tracking-wide border shrink-0" :class="getStatusBadge(update.status)">
                  ...
                  <div v-if="update.status === 'resolved' && (update.unit === 'FGMU' || update.unit === 'LEAU')" class="mt-2.5 flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-amber-50 border border-amber-200/80 text-amber-800">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 shrink-0 text-amber-600" viewBox="0 0 20 20" fill="currentColor">
                      <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd" />
                    </svg>
                    <p class="text-xs font-bold">Action Required: Tap here to rate and close</p>
                  </div>
                </div>
              </div>
