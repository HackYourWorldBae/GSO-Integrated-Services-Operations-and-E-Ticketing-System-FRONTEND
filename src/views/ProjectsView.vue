<template>
  <div class="projects-page min-h-screen bg-slate-50 flex flex-col font-sans">
    ...
    <nav class="bg-white/80 backdrop-blur-lg sticky top-0 z-50 border-b border-slate-100 shadow-sm">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-20 items-center">
          ...
          <div class="hidden md:flex items-center gap-8">
            ...
            <router-link to="/projects" class="text-sm font-bold text-emerald-600 border-b-2 border-emerald-600 pb-1">Projects</router-link>
            <router-link to="/projects/archives" class="text-sm font-bold text-slate-500 hover:text-emerald-600 transition-colors">Archives</router-link>
      ...
      <div v-show="isMobileMenuOpen" class="md:hidden bg-white border-t border-slate-100 absolute w-full shadow-xl animate-fade-in-down">
        <div class="px-4 pt-2 pb-6 space-y-2">
          ...
          <router-link to="/projects" @click="isMobileMenuOpen = false" class="block px-3 py-3 text-base font-bold text-emerald-600 bg-emerald-50 rounded-xl">Projects</router-link>
          <router-link to="/projects/archives" @click="isMobileMenuOpen = false" class="block px-3 py-3 text-base font-bold text-slate-600 hover:bg-slate-50 rounded-xl">Archives</router-link>
    ...
    <header class="bg-slate-900 py-16 sm:py-24 relative overflow-hidden shrink-0">
      ...
      <div class="absolute -top-24 -right-24 w-96 h-96 bg-emerald-500 rounded-full blur-3xl opacity-20 animate-pulse"></div>
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
        <h1 class="text-4xl sm:text-5xl lg:text-6xl font-black text-white tracking-tight mb-4 leading-tight">
          Campus <span class="text-emerald-400">Projects</span>
        </h1>
        <p class="mt-4 max-w-2xl text-lg sm:text-xl text-slate-300 mx-auto font-medium">
          Stay informed about ongoing facility upgrades, maintenance schedules, and groundskeeping operations across the university.
        </p>
      </div>
    </header>
    ...
    <main class="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 w-full">
      ...
      <div class="flex flex-col sm:flex-row justify-between items-center mb-10 gap-4">
        <h2 class="text-2xl font-black text-slate-900">Active & Upcoming</h2>
      ...
      <div v-if="loading" class="text-center py-20">
        ...
        <p class="text-slate-500 font-bold tracking-wider uppercase text-sm">Loading Projects...</p>
      </div>
      ...
      <div v-else-if="filteredProjects.length === 0" class="text-center py-24 bg-white rounded-[2rem] border border-slate-200 shadow-sm">
        ...
        <h3 class="text-xl font-black text-slate-900 mb-2">No active projects found</h3>
        <p class="text-slate-500 font-medium">There are currently no announcements matching this filter.</p>
      </div>
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        <div v-for="project in filteredProjects" :key="project.id" class="group bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-300 flex flex-col hover:-translate-y-1">
          <!-- Card Header -->
          <div :class="['px-6 py-4 flex justify-between items-center', project.unit_id === 1 ? 'bg-emerald-600' : 'bg-amber-500']">
          ...
          <div class="p-6 flex-1 flex flex-col">
            <h3 class="text-xl font-black text-slate-900 leading-tight mb-2 group-hover:text-emerald-600 transition-colors">{{ project.project_title }}</h3>
            ...
            <div class="bg-slate-50 rounded-2xl p-4 border border-slate-100 space-y-3 mb-6 flex-1">
              ...
              <div class="flex justify-between items-center text-sm">
                ...
                <span class="px-2 py-0.5 bg-blue-100 text-blue-700 font-bold rounded text-xs uppercase tracking-wider animate-pulse">In Progress</span>
              </div>
            </div>

            <!-- Expandable Remarks -->
            <div v-if="project.project_remarks" class="mt-auto">
              <details class="group/details">
                <summary class="text-sm font-bold text-emerald-600 cursor-pointer list-none flex items-center justify-between hover:text-emerald-700 select-none">
                  View Remarks & Scope
                ...
                <div class="mt-3 text-sm text-slate-600 bg-emerald-50/50 p-4 rounded-xl border border-emerald-100/50 italic leading-relaxed">
                  {{ project.project_remarks }}
                </div>
              </details>
            </div>
            <div v-else class="mt-auto pt-4 border-t border-slate-100">
              <p class="text-xs text-slate-400 font-medium">No additional remarks provided.</p>
            </div>
          </div>
          
          <!-- Card Footer -->
          <div class="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-between items-center">
            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Announced</span>
            <span class="text-xs font-bold text-slate-600">{{ formatDate(project.submitted_at) }}</span>
          </div>
        </div>
      </div>
    </main>
